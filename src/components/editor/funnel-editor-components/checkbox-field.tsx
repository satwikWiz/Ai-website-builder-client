...
const CheckboxField = ({ element }: Props) => {
  ...
  return (
    <div
      ...
      onDragStart={!state.editor.previewMode && !state.editor.liveMode ? (e) => handleDragStart(e, 'checkbox') : undefined}
      onClick={!state.editor.previewMode && !state.editor.liveMode ? handleOnClickBody : undefined}
      className={clsx(
        'w-full relative transition-all flex items-center',
        {
    ...
    >
      ...
      <input type="checkbox" className="mr-2" readOnly={state.editor.liveMode || state.editor.previewMode} />
      <span>Checkbox</span>
      {state.editor.selectedElement.id === element.id && !state.editor.liveMode && !state.editor.previewMode && (
